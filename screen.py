...
class KlipperScreen(Gtk.Window):
    ...
    def __init__(self, args, version):
        ...
        self.width = 800 #self._config.get_main_config().getint("width", monitor.get_geometry().width)
        self.height = 600 #self._config.get_main_config().getint("height", monitor.get_geometry().height)
        ...
        show_cursor = True #self._config.get_main_config().getboolean("show_cursor", fallback=False)
    ...
    def ws_subscribe(self):
        ...
        for fs in self.printer.get_filament_sensors():
            requested_updates['objects'][fs] = ["enabled", "filament_detected"]
            requested_updates['objects'][fs] = ["filament_presence_sens_1", "filament_presence_sens_2"]
        for p in self.printer.get_output_pins():
            requested_updates['objects'][p] = ["value"]
        for e in self.printer.get_extra_pins():
            requested_updates['objects'][e] = ["board", "cmd"]
        for be in self.printer.get_box_endstop():
            requested_updates['objects'][be] = ["state"]
    ...
    def _websocket_callback(self, action, data):
        ...
        self.process_update(action, data)
                #### added code #####
        #self.show_popup_message(str(self.printer.get_status_optical_sensor('extruder1', sensor=2)))
        if self.printer.get_printing_status() == "printing" and not self.printer.get_status_optical_sensor(self.printer.get_stat("toolhead", "extruder"), sensor=1):
            if not self.printer.get_pause_status():
                if self.send_count < 1:
                    self.show_popup_message("Пауза печати! Загрузите филамент!", 4)
            ...
            self.send_count = 0
        #self.path = self.printer.get_path_gcodefolder()
        ####### end added code #######
    ...
    def init_printer(self):
        ...
        extra_items = (self.printer.get_tools()
                       ...
                       + self.printer.get_extra_pins()
                       + self.printer.get_box_endstop()
                       )
