...
class Printer:
    ...
    def process_update(self, data):
        for x in (self.get_tools() + self.get_heaters() + self.get_filament_sensors() + self.get_output_pin() + self.get_extra_pins() + self.get_box_endstop()):
            if x in data:
                for i in data[x]:
    ...
    #added code 
    def get_box_endstop(self):
        endstop = []
        for h in self.get_config_section_list("box_endstop"):
            endstop.append(h)
        return endstop

    def get_output_pin(self):
        pins = []
        for h in self.get_config_section_list("output_pin "):
            pins.append(h)
        return pins

    def get_extra_pins(self):
        extra_pins = []
        for h in self.get_config_section_list("extra_pin "):
            extra_pins.append(h)
        return extra_pins

    def get_home_status(self):
        self.count = 0
        status = self.data['motion_report']['live_position']
        for i in status:
            if i == 0.:
                self.count += 1
    ...
    def get_pinout_status(self, option):
        return self.data['output_pin ' + str(option)]['value']

    def is_pin_on_control_board(self, option):
        if self.data['extra_pin ' + str(option)]['board'] == 'control_board':
            return True
        else:
            return False

    def get_cmd_extra_pin(self, option):
        return self.data['extra_pin ' + str(option)]['cmd']
    
    def get_box_endstop_status(self):
        return self.data['box_endstop']['state']

    def toggle_extra_pin(self, option):
        self.toggle = self.data['extra_pin ' + str(option)]['stat']
        self.toggle = not self.toggle 
        self.data['extra_pin ' + str(option)]['stat'] = int(self.toggle)
        return int(self.toggle)
    
    def get_status_optical_sensor(self, extruder, sensor):
        if sensor == 1:
            sensor = 'filament_presence_sens_1'
        if sensor == 2:
            sensor = 'filament_presence_sens_2'
        if extruder == 'extruder':
            extruder = 'filament_presence_sensor extruder1'
        if extruder == 'extruder1':
            extruder = 'filament_presence_sensor extruder2'
        if extruder in self.data:
            return self.data[extruder][sensor]
    
    def get_pause_status(self, pause_resume='pause_resume', is_paused='is_paused'):
        if pause_resume in self.data:
            return self.data[pause_resume][is_paused]

    def get_printing_status(self, print_stats='print_stats', state='state'):
        if print_stats in self.data:
            return self.data[print_stats][state].lower()
        else:
            return False

    def get_path_gcodefolder(self, section='flash_card_usb'):
        if section in self.get_config_section_list():
            return self.config[section]['gcode_files_path']
        else:
            return False

    def get_devices_temp(self, device):
        if device in self.devices:
            return self.devices[device]["temperature"]
        else:
            return False
    # end added code
    ...
    def get_filament_sensors(self):
        ...
        sensors.extend(iter(self.get_config_section_list("filament_presence_sensor ")))
        return sensors
