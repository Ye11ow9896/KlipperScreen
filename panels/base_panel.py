...
class BasePanel(ScreenPanel):
    def __init__(self, screen, title):
        ...
        ## added code 
        self.control['boxlight'] = self._gtk.Button('light', scale=abscale)
        self.control['boxlight'].connect("clicked", self.boxlight)
        self.control['fanhoods'] = self._gtk.Button('fan', scale=abscale)
        self.control['fanhoods'].connect("clicked", self.fanhoods)
        # end added code 
        ...
        #added code
        self.action_bar.add(self.control['boxlight'])
        self.action_bar.add(self.control['fanhoods'])
        #end added code
        ...
        # added code 
        self.control['door_stat'] = Gtk.Box()
        self.control['door_stat'].set_halign(Gtk.Align.CENTER)
        # end added code
        ...
        self.titlebar.add(self.control['door_stat'])
    ...
    def show_heaters(self, show=True):
        try:
            ...
            if self._screen.printer.get_tools():
                if 'extruder' and 'extruder_box' in self.labels:
                    self.control['temp_box'].add(self.labels['extruder_box'])
                    n += 1
                if 'extruder1' and 'extruder1_box' in self.labels:
                    self.control['temp_box'].add(self.labels['extruder1_box'])
                    n += 1
    ...
    def get_icon(self, device, img_size):
        if device.startswith("extruder"):
            self.countextr = self._screen.printer.extrudercount 
            if self._screen.printer.extrudercount > 1:
                if device == "extruder":
                    self.sf = self._screen.printer.get_status_optical_sensor(device, sensor=1) 
                    if not self._screen.printer.get_status_optical_sensor(device, sensor=1):
                        return self._gtk.Image("extruder-0", img_size, img_size)
                    else:
                        return self._gtk.Image("klipper", img_size, img_size)
                elif device == "extruder1":
                    if not self._screen.printer.get_status_optical_sensor(device, sensor=1):
                        return self._gtk.Image("extruder-1", img_size, img_size)
                    else:
                        return self._gtk.Image("klipper", img_size, img_size)
            #if self._screen.printer.extrudercount > 1:
            #    if device == "extruder":
    ...
    # added code 
    def boxlight(self, widget):
        self.cmd = self._screen.printer.get_cmd_extra_pin('boxlight')

        if self._screen.printer.is_pin_on_control_board('boxlight'): # control board
            self._screen._ws.klippy.gcode_script(self.cmd)
        else: # sbc
            self.val = self._screen.printer.toggle_extra_pin('boxlight')
            self.command = str(self.cmd) + str(self.val) 
            os.system(self.command)

    def fanhoods(self, widget):
        self.cmd = self._screen.printer.get_cmd_extra_pin('fanhoods')
        
        if self._screen.printer.is_pin_on_control_board('fanhoods'): # control board
            self._screen._ws.klippy.gcode_script(self.cmd)
        else: # sbc
            self.val = self._screen.printer.toggle_extra_pin('fanhoods')
            self.command = str(self.cmd) + str(self.val) 
            os.system(self.command)

    def _update_panel_icons(self):
        for child in self.control['temp_box'].get_children():
            self.control['temp_box'].remove(child)
        img_size = self._gtk.img_scale * self.bts
        for device in self._screen.printer.get_temp_store_devices():
            if device.startswith("extruder"):
                if self._screen.printer.extrudercount > 1:
                    if device == "extruder":
                        self.st = self._screen.printer.get_status_optical_sensor(device, sensor=1)
                        if not self._screen.printer.get_status_optical_sensor(device, sensor=1):
                            icon = self._gtk.Image("extruder-0", img_size, img_size)
                        else:
                            icon = self._gtk.Image("klipper", img_size, img_size)    
                    else:
                        if not self._screen.printer.get_status_optical_sensor(device, sensor=1):
                            icon = self._gtk.Image("extruder-%s" % device[8:], img_size, img_size)
                        else:
                            icon = self._gtk.Image("klipper", img_size, img_size)    
                else:
                    if not self._screen.printer.get_status_optical_sensor(device, sensor=1):
        ...
        if self._screen.printer.get_tools():
            ...
            self.control['temp_box'].add(self.labels["%s_box" % "extruder"])
            n += 1
            # edd next extruder, if their anount more 1
            if self._screen.printer.extrudercount > 1:
                self.control['temp_box'].add(self.labels["%s_box" % "extruder1"])
                n += 1
    ...
    def _update_door_status(self):
        img_size = self._gtk.img_scale * self.bts
        for child in self.control['door_stat'].get_children():
            self.control['door_stat'].remove(child)
        self.status_box = self._screen.printer.get_box_endstop_status()
        if self.status_box == 1:
            icon = self._gtk.Image("warning", img_size, img_size)
        else:
            icon = self._gtk.Image("home-x", img_size, img_size)
        self.labels['door_stat'] = Gtk.Box()
        self.labels['door_stat'].pack_start(icon, False, False, 3)
        self.control['door_stat'].add(self.labels['door_stat'])
        self.control['door_stat'].show_all()
    # end added code
