...
class ExtrudePanel(ScreenPanel):
    ...
    def __init__(self, screen, title):
        ...
        self.load_fil = False
        self.unload_fil = False
        
        self.speeds = ['1', '2', '5', '25']
        self.distances = ['5', '10', '15', '25']

        ##### added code   #########
        self.conf = self._config.get_configurable_options()
        for item in self.conf:
            name = list(item)[0]
            if name == 'ret_length':
                opt = item[name]
                self.ret_length = int(self._config.get_config().get(opt['section'], name, fallback=opt['value']))
                
            if name == 'ret_speed':
                opt = item[name]
                self.ret_speed = int(self._config.get_config().get(opt['section'], name, fallback=opt['value']))
                
            if name == 'tube_length_extr1':
                opt = item[name]
                self.tube_length_extr0 = int(self._config.get_config().get(opt['section'], name, fallback=opt['value']))
                
            if name == 'tube_length_extr2':
                opt = item[name]
                self.tube_length_extr1 = int(self._config.get_config().get(opt['section'], name, fallback=opt['value']))  

            if name == 'unret_extra_length':
                opt = item[name]
                self.unret_extra_length = int(self._config.get_config().get(opt['section'], name, fallback=opt['value']))
          
            if name == 'unret_speed':
                opt = item[name]
                self.unret_speed = int(self._config.get_config().get(opt['section'], name, fallback=opt['value']))
        ##### end added code #######
        ...
        self.buttons = {
            ...
            # added code setting button
            'settings': self._gtk.Button("settings", _("Settings"), "color4"),
            ##end added code
        }
        self.buttons['extrude'].connect("clicked", self.extrude, "+")
        self.buttons['load'].connect("clicked", self._load_filament) # "+")
        self.buttons['unload'].connect("clicked", self._unload_filament) #, "-")
        ...
        #added connect to settings button
        self.buttons['settings'].connect("clicked", self.menu_item_clicked, "setting", {
            "name": "Filament setting",
            "panel": "filament_settings"
        })
        #end adding code
        ...
        extgrid.attach(self.buttons['settings'], i + 2, 0, 1, 1)
    ...
    def change_extruder(self, widget, extruder):
        ...
        self._screen._ws.klippy.gcode_script("ACTIVATE_EXTRUDER EXTRUDER=%s" % extruder)
...
### added code ###
    def _load_filament(self, widget):
        if self._printer.get_home_status():
            if self.optical_sens_timeout is None:
                self.optical_sens_timeout = GLib.timeout_add(500, self._check_optical_sensors)
            self.current_extruder = self._printer.get_stat("toolhead", "extruder")
            if self._printer.get_status_optical_sensor(self.current_extruder, sensor=1):
                if self._printer.get_status_optical_sensor(self.current_extruder, sensor=2):
                    self._screen.show_popup_message('Филамент уже загружен!',3)
                else: 
                    if self._printer.get_devices_temp(self.current_extruder) >= 170.:
                        self.load_fil = True
                    else:
                        self._screen.show_popup_message('Низкая температура. Нагрейте экструдер!',3)
                        self.load_fil = True
                        self.menu_item_clicked(None, "temperature", {
                          "name": "Temperature",
                          "panel": "temperature"
                        })
            else: 
                    self._screen.show_popup_message('Загрузите филамент!',3)
        else:
            self._screen.show_popup_message('Необходимо выйти в домашнee положение!')
    ...
    def _unload_filament(self, widget):
        if self._printer.get_home_status():
            ...
            if self._printer.get_devices_temp(self.current_extruder) >= 170.:
                if self._printer.get_status_optical_sensor(self.current_extruder, sensor=2):
                    self.unload_fil = True
                else:
                    self._screen.show_popup_message('Филамент уже был выгружен!',3)
            else:
                self._screen.show_popup_message('Низкая температура. Нагрейте экструдер!',3)
                self.unload_fil = True
                self.menu_item_clicked(None, "temperature", {
            ...
            self._screen.show_popup_message('Необходимо выйти в домашнее положение!')

    def _check_optical_sensors(self):
        self.current_extruder = self._printer.get_stat("toolhead", "extruder")
        if self.load_fil == True:
            if self.check_temp_heating():
                if not self._printer.get_status_optical_sensor(self.current_extruder, sensor=2):
                    self._screen._ws.klippy.gcode_script('M83')
                    self._screen._ws.klippy.gcode_script('G1 E1 F400')
                else: 
                    self._screen._ws.klippy.gcode_script('M83')
                    if self.current_extruder == 'extruder':
                        self._screen._ws.klippy.gcode_script('G1 E' + str(self.tube_length_extr0) + ' F2000')
                    if self.current_extruder == 'extruder1':
                        self._screen._ws.klippy.gcode_script('G1 E' + str(self.tube_length_extr1) + ' F2000')
                    self._screen._ws.klippy.gcode_script('NOZZLE_CLEAN')
                    self.load_fil = False

        if self.unload_fil == True:
            if self.check_temp_heating():
                if self._printer.get_status_optical_sensor(self.current_extruder, sensor=2):
                    self._screen._ws.klippy.gcode_script('M83')
                    self._screen._ws.klippy.gcode_script('G1 E-10 F1500')
                if not self._printer.get_status_optical_sensor(self.current_extruder, sensor=2):
                    self._screen._ws.klippy.gcode_script('M83')
                    self._screen._ws.klippy.gcode_script('G1 E-40 F1000')
                    self._screen.show_popup_message('Выгрузка завершена!',3)
                    self.unload_fil = False
        return True

    def check_temp_heating(self):
        self.current_extruder = self._printer.get_stat("toolhead", "extruder")
        self.tar = self._printer.get_dev_stat(self.current_extruder, 'target')
        self.cur =  self._printer.get_dev_stat(self.current_extruder, 'temperature') + 2
        if self.cur >= 170.:
            if self.cur <= self.tar:
                return False
            else: 
                panel = self._screen._cur_panels[-1]
                if panel == 'main_panel_actions_extrude_temperature':
